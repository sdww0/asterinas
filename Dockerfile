...
# RUN make -j \ 
#     && mkdir -p /usr/local/benchmark/membench \ 
...
# Clear cached files
# WORKDIR /root
# RUN rm -r --force sysbench-1.0.20 \ 
#     membench \ 
#     iperf \
#     lmbench \
#     byte-unixbench \
...
#     hackbench \
#     schbench \
#     libmemcached-1.0.18/libhashkit-1.0 \
#     libmemcached-1.0.18/libmemcachedinternal \
#     libmemcached-1.0.18
...
RUN wget https://www.sqlite.org/2024/sqlite-amalgamation-3460100.zip \
    ...
    && mv ./libsqlite3.so /lib/riscv64-linux-gnu/ \
    && mv ./sqlite3.h /usr/include/riscv64-linux-gnu/ \
    && mv ./sqlite3ext.h /usr/include/riscv64-linux-gnu/

RUN rm -r -f sqlite-amalgamation-3460100.zip \
...
# RUN rm -r --force redis-7.0.15/utils/lru

# RUN rm -r --force redis-7.0.15.tar.gz \ 
#     redis-7.0.15 
...
# RUN rm -r --force memcached-1.6.32.tar.gz \
#     memcached-1.6.32
...
RUN apt update && apt-get install -y --no-install-recommends \ 
    ...
    meson \ 
    ninja-build
RUN apt clean && rm -r --force /var/lib/apt/lists/*
...
RUN wget -O qemu.tar.xz https://download.qemu.org/qemu-9.1.0.tar.xz \ 
    ...
    && rm qemu.tar.xz
WORKDIR /root/qemu
RUN ./configure --target-list=riscv64-softmmu --prefix=/usr/local/qemu --enable-slirp \ 
    && make -j \ 
    && make install
WORKDIR /root
# RUN rm -r --force /root/qemu
...
RUN curl https://sh.rustup.rs -sSf | \ 
    ...
    && rm -r --force /root/.cargo/registry && rm -r --force /root/.cargo/git \ 
    && cargo -V \ 
...
ENV LD_LIBRARY_PATH="/usr/local/qemu/lib/riscv64-linux-gnu:${LD_LIBRARY_PATH}"
...
COPY --from=build-applications /lib/riscv64-linux-gnu/libsqlite3.so /lib/riscv64-linux-gnu/libsqlite3.so
