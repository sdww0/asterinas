...
impl FileIo for VirtioBlk {
    ...
    fn ioctl(&self, cmd: IoctlCmd, arg: usize) -> Result<i32> {
        if cmd == IoctlCmd::FIOBLKGETSIZE64 {
            write_val_to_user(arg, &self.device.metadata().capacity);
            info!("Capacity:{:x?}", self.device.metadata().capacity);
            return Ok(0);
        }
        error!("Unimplemented cmd:{:?}", cmd);
        return_errno_with_message!(Errno::EINVAL, "Unsupport cmd");
    }
}
